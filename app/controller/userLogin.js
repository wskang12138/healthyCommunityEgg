...
class LoginController extends Controller {
  async login() {
    ...
    if (
      `'${global.userCode}'`.toLowerCase() !==
      `'${query.captcha}'`.toLowerCase()
    ) {
      return (ctx.body = ctx.helper.responseCode(false, "验证码错误", "", 500));
    }
    const result = await ctx.service.userLogin.login(query);
  ...
  async register() {
    ...
    if (result) {
      ...
      const res = await ctx.service.userLogin.register(query);
      if (res) {
        return (ctx.body = ctx.helper.responseCode(true, "注册成功", "", 200));
      } else {
        return (ctx.body = ctx.helper.responseCode(false, "注册失败", "", 200));
      }
    }
  }
  //用户注销
  ...
  async captcha() {
    const { ctx } = this;
    let captcha = await ctx.service.userLogin.captcha(); // 服务里面的方法
